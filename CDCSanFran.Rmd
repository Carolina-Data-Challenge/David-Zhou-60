...
library(ggmap)
```
  Read in the air pollution data for San Francisco, CA.
```{r}
...
```
Using the dataset that was given, the data was filtered into each type of pollutant, and then the average pollution level for each pollutant was calculated. 
```{r}
...
```
  The data was used to creat a boxplot for each pollutant. Each data point was plotted on a logarithmic scale to adjust the scale of the plots. From the boxplots, it is evident that there are outliers for several of the pollutants. For example, the average pollution level for O3 is relatively low, however there are several times that do have values much higher than this average.

Note: For this dataset, you will notice there is no boxplot present for PM10. This is because there is no recorded data for black carbom in San Francisco, CA.
```{r}
...
figure
```
  Next, the mean pollution levels of the pollutants were plotted. This was compared to national data to assess pollution levels in San Francisco. 

  From the bar graphs that show the national air quality standard in comparison to the air quality data for San Francisco, CA, it is clear to see that San Francisco is below the national standard in every pollutant category.
...
mean(noon_no2$Value)
```
  Here measurements that were taken at 12 pm UTC were plotted across time. The average value of the pollutant was plotted as a horizontal line across the graph to be referenced. 
